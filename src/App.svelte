<script>
  ...
  $: params.symmetry =
    params.symmetry === "h" && params.data_type === "d" ? "s" : params.symmetry;
  // there is no banded polynomial solver
  $: params.prob_type =
    params.prob_type === "pev" && params.storage_format === "banded"
      ? "ev"
      : params.prob_type;
  // IFEAST and PFEAST only for sparse
  $: params.algorithm =
    params.storage_format !== "sparse" ? "standard" : params.algorithm;

  const fpm_init = {
    //given values are feastinit defaults
  ...
  function fpm_default(parameters, fparam) {
    ...
    if (
      parameters.algorithm !== "inexact" &&
      parameters.algorithm !== "both" &&
      parameters.prob_type !== "pev" &&
      ((parameters.symmetry === "s" && parameters.data_type === "d") ||
        parameters.symmetry === "h")
    ) {
      defaults[18] = 30;
    } else {
      defaults[18] = 100;
  ...
  function fpm_set_default_fpm(p) {
    switch (p) {
      ...
      case 18:
        fpm[18] =
          !is_ifeast(params) &&
          params.prob_type !== "pev" &&
          (is_hermitian(params) ||
            (params.data_type === "d" && params.symmetry === "s"))
            ? 30
            : 100;
        break;
      default:
  ...
  function is_ifeast(parameters) {
    return (
      parameters.algorithm === "inexact" || parameters.algorithm === "both"
    );
  }
  ...
  function is_real(parameters) {
    if (parameters.prob_type === "pev") {
      ...
      return (
        (parameters.data_type === "d" && parameters.symmetry === "s") ||
        (parameters.data_type === "z" && parameters.symmetry === "h")
      );
    }
  }
  ...
  function feast_call(parameters) {
    ...
    let P = "ev";
    let PFEAST =
      parameters.algorithm === "parallel" || parameters.algorithm === "both"
        ? "p"
        : "";
    let IFEAST =
      parameters.algorithm === "inexact" || parameters.algorithm === "both"
  ...
  function feast_arguments(parameters) {
    ...
    listi =
      !is_real(parameters) || parameters.prob_type === "pev"
        ? "Emid,r"
        : "Emin,Emax";
...
<main class="section">
    ...
    <div class="columns is-mobile">
      <div class="column">
        ...
        <p class="content is-marginless">
          See the <a href="http://www.ecs.umass.edu/~polizzi/feast/">homepage</a
          > of the FEAST eigenvalue solver for details and documentation.
        </p>
        <p class="content">
          Configure FEAST interface before input parameters. Currently only
          supports version 4.0.
        </p>
        <p>Configurator version 4.0.0 - February 12th 2020.</p>
      </div>
  ...
  <div class="container">
    <h2 class="title is-4">FEAST Interfaces</h2>
    ...
    <!-- data type -->
    <InterfaceSelect
      bind:value={params.data_type}
      on:change={() => fpm_set_default_param()}
      description="Matrix data type"
    >
      <option
        value={"d"}
        disabled={(params.storage_format === "rci" &&
          params.prob_type === "pev") ||
          null}>Double Precision (d)</option
      >
    ...
    <!-- storage format -->
    <InterfaceSelect
      bind:value={params.storage_format}
      on:change={() => fpm_set_default_param()}
    ...
    <!-- problem symmetry -->
    <InterfaceSelect
      bind:value={params.symmetry}
      on:change={() => fpm_set_default_param()}
      description="Symmetry of problem"
    >
      <option value={"s"}>Symmetric (s)</option>
      <option value={"h"} disabled={params.data_type === "d" || null}
        >Hermitian (h)</option
      >
    ...
    >
      ...
      <option
        value={"pev"}
        disabled={params.storage_format === "banded" || null}
        >Polynomial (pev)</option
      >
    </InterfaceSelect>

    <!-- algorithm type -->
    <InterfaceSelect
      bind:value={params.algorithm}
    ...
    >
      ...
      <option
        value={"inexact"}
        disabled={params.storage_format !== "sparse" || null}>IFEAST</option
      >
      <option
        value={"parallel"}
        disabled={params.storage_format !== "sparse" || null}>PFEAST</option
      >
      <option
        value={"both"}
        disabled={params.storage_format !== "sparse" || null}>PIFEAST</option
      >
    </InterfaceSelect>

    <!-- version -->
    <InterfaceSelect
      bind:value={params.expert}
  ...
  <div class="container">
    <div class="columns is-mobile">
      <div class="column is-10">
        <h2 class="title is-4">Runtime Options</h2>
      </div>
      <div class="column is-small">
        <button
          class="button is-fullwidth"
          on:click={() => (runtime_visable = !runtime_visable)}
        >
          <span>{runtime_visable ? "Hide" : "Show"}</span>
          <span class="icon is-small">
            <i
              class="arrow {runtime_visable ? 'up' : 'down'}"
              aria-hidden="true"
            ></i>
          </span>
        </button>
    ...
    {#if runtime_visable}
      <div transition:slide>
        ...
        <FPMSelect
          ...
          fpmIndex={14}
          on:change={() => {
            fpm_set_default_fpm(2);
            fpm_set_default_fpm(8);
            fpm_set_default_fpm(15);
          }}
    ...
    <div class="columns is-mobile">
      <div class="column is-10">
        <h2 class="title is-4">Integration Options</h2>
      </div>
      <div class="column is-small">
        <button
          class="button is-fullwidth has-dropdown"
          on:click={() => (contour_visable = !contour_visable)}
        >
          <span class="has-text-left">{contour_visable ? "Hide" : "Show"}</span>
          <span class="icon is-small">
            <i
              class="arrow {contour_visable ? 'up' : 'down'}"
              aria-hidden="true"
            ></i>
          </span>
        </button>
    ...
    {#if contour_visable}
      <div transition:slide>
        ...
        >
          ...
          <option value={2} disabled={!is_real(params) || null}
            >Zolotarev</option
          >
        </FPMSelect>
    ...
    <div class="columns is-mobile">
      <div class="column is-10">
        <h2 class="title is-4">Driver Options</h2>
      </div>
      <div class="column is-small">
        <button
          class="button is-fullwidth has-dropdown"
          on:click={() => (driver_visable = !driver_visable)}
        >
          <span class="has-text-left">{driver_visable ? "Hide" : "Show"}</span>
          <span class="icon is-small">
            <i class="arrow {driver_visable ? 'up' : 'down'}" aria-hidden="true"
            ></i>
          </span>
        </button>
      </div>
    ...
    {#if driver_visable}
      <div transition:slide>
        ...
        <FPMInteger
          ...
          min="0"
          max="10"
          disabled={!is_ifeast(params) || null}
    ...
    <div class="columns">
      <div class="column control">
        <textarea
          ...
          rows={param_diff.count + 1}
          readonly
          >{param_diff.text.trim()}
    ...
    <InterfaceTable
      ...
      real={is_real(params)}
      probType={params.prob_type}
      expert={params.expert}
      storageFormat={params.storage_format}
    />
  </div>
</main>
...
<footer class="footer">
  <div class="content has-text-centered">
    <p>
      If you find any bugs, or have suggestions, please send an email or open an
      issue on <a href="https://github.com/spacedome/feast-config">github</a>.
    </p>
    <p>Julien Brenneck &middot 2020</p>
  </div>
</footer>
